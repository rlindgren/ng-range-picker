module.exports = {
  ...
  controller: ['$scope', '$element', '$timeout', 'rangePickerConfig', function ($scope, $element, $timeout, rangePickerConfig) {
    ...
    this.getDays = (date) => {
      ...
      let start = moment(this.ngModel.startDate).startOf('day').hours(12);
      let end = moment(this.ngModel.endDate).startOf('day').hours(12);
      for (var i = 0; i < displayedDays; i++) {
        let mo = moment(date).startOf('day').hours(12).date(i - startDay + 1);
        days.push({
