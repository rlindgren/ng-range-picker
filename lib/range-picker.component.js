module.exports = {
  ...
  controller: ['$scope', '$element', '$timeout', 'rangePickerConfig', 'rangePickerDelegate', function ($scope, $element, $timeout, rangePickerConfig, rangePickerDelegate) {
    ...
    this.$onInit = () => {
      ...
      var routeChangeWatch = this.preserveOnRouteChange ? this.noop : $scope.$on('$routeChangeStart', () => { 
        rangePickerDelegate.hideAll();
      });

      var stateChangeWatch = this.preserveOnStateChange ? this.noop : $scope.$on('$stateChangeStart', () => { 
        rangePickerDelegate.hideAll();
      });

      var locationChangeWatch = this.preserveOnLocationChange ? this.noop : $scope.$on('$locationChangeStart', () => { 
        rangePickerDelegate.hideAll();
      });
